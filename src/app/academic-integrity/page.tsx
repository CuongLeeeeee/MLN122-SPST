  ...
  const copilotSessions: AIToolSession[] = [
    {
      ...
      prompt:
        "Tạo game canvas 2D với top-down view, player movement sử dụng WASD và phím mũi tên, bao gồm Game class, Player class, Input system, Maps system với doors và interactive objects",
      aiOutput:
        "Copilot đã tạo ra toàn bộ cấu trúc game engine với Player class (movement, collision), Input system (keyboard handling), render loop, Maps.ts với các hàm tạo map/door/exhibit objects, và logic chuyển map. Code có cấu trúc rõ ràng với TypeScript types đầy đủ.",
      teamEdits:
        "Team đã chỉnh sửa và mở rộng: điều chỉnh vị trí shadow của player, tối ưu hóa collision detection, thêm smooth camera movement, cải thiện door placement logic, và tối ưu render performance cho nhiều objects",
    },
    {
      tool: "GitHub Copilot (ChatGPT 4)",
      prompt:
        "Tạo quiz modal component với multiple choice questions, answer validation, và animation khi chuyển câu hỏi",
      aiOutput:
        "Copilot sinh ra QuizModal component hoàn chỉnh với state management cho current question, answer selection, answer validation với correct/incorrect feedback, và basic styling với modal overlay",
      teamEdits:
        "Team đã tùy chỉnh và cải thiện: thêm sound effects cho correct/incorrect answers, cải thiện animation transitions giữa các câu hỏi, tích hợp với game state để lưu progress, và thêm quiz completion flow",
    },
  ];
  ...
  const notebookLMSessions: AIToolSession[] = [
    {
      tool: "NotebookLM",
      prompt:
        "Phân tích và tổng hợp khái niệm 'Nhà nước xã hội chủ nghĩa' từ tài liệu Hiến pháp 2013 và giáo trình MLN131",
      aiOutput:
        "NotebookLM đã phân tích các nguồn và tổng hợp: Nhà nước XHCN Việt Nam là nhà nước của nhân dân, do nhân dân và vì nhân dân. Quyền lực nhà nước là thống nhất, có sự phân công, phối hợp, kiểm soát giữa các cơ quan trong việc thực hiện các quyền lập pháp, hành pháp, tư pháp.",
      teamEdits:
        "Team đã tách các khái niệm chính thành các điểm riêng biệt, tạo cấu trúc phân cấp cho dễ hiểu trong flipbook và thiết kế câu hỏi quiz kiểm tra từng khía cạnh",
    },
    {
      tool: "NotebookLM",
      prompt:
        "Tổng hợp nội dung về 'Dân chủ xã hội chủ nghĩa' từ văn kiện Đại hội XIII và tài liệu học tập",
      aiOutput:
        "NotebookLM tổng hợp: Dân chủ XHCN là bản chất của chế độ ta, được thực hiện thông qua các hình thức dân chủ trực tiếp (trưng cầu dân ý, hội nghị cử tri) và dân chủ đại diện (bầu cử đại biểu). Nhân dân có quyền tham gia quản lý nhà nước và xã hội, được bảo đảm quyền giám sát hoạt động của các cơ quan nhà nước.",
      teamEdits:
        "Team đã đơn giản hóa ngôn ngữ học thuật, thêm ví dụ thực tế về các hình thức dân chủ để người chơi dễ liên hệ với đời sống",
    },
  ];
  ...
  const chatGPTSessions: AIToolSession[] = [
    {
      tool: "ChatGPT 4",
      prompt:
        "Giải thích khái niệm 'Nhà nước pháp quyền xã hội chủ nghĩa' và các đặc điểm cơ bản",
      aiOutput:
        "ChatGPT giải thích: Nhà nước pháp quyền XHCN là nhà nước do Hiến pháp và pháp luật điều chỉnh, trong đó Hiến pháp có hiệu lực pháp lý cao nhất. Đặc điểm: tôn trọng và bảo đảm quyền con người, bảo đảm tính minh bạch và trách nhiệm giải trình, có cơ chế kiểm soát quyền lực để phòng chống lạm quyền và tham nhũng.",
      teamEdits:
        "Team đã cấu trúc lại thành 4 đặc điểm chính với giải thích ngắn gọn cho từng đặc điểm, tạo câu hỏi quiz dạng matching và multiple choice",
    },
    {
      tool: "ChatGPT 4",
      prompt:
        "Mối quan hệ giữa dân chủ XHCN và nhà nước pháp quyền XHCN ở Việt Nam",
      aiOutput:
        "ChatGPT phân tích: Dân chủ XHCN và nhà nước pháp quyền XHCN có mối quan hệ gắn bó hữu cơ. Dân chủ là nội dung, pháp quyền là hình thức và phương thức bảo đảm. Nhà nước pháp quyền tạo khuôn khổ pháp lý để thực hiện dân chủ, trong khi dân chủ là mục tiêu và động lực phát triển nhà nước pháp quyền.",
      teamEdits:
        "Team đã đơn giản hóa mối quan hệ phức tạp này thành sơ đồ minh họa và các câu hỏi quiz giúp người chơi hiểu được sự tương tác giữa hai khái niệm",
    },
  ];
    ...
    <section className={styles.section}>
      ...
      <div className={styles.toolCategory}>
        ...
        <p className={styles.toolDescription}>
          Sử dụng để hỗ trợ viết code và phát triển giao diện game. Copilot đã
          giúp tạo các component React, game logic, và tích hợp API.
        </p>
      ...
      <div className={styles.toolCategory}>
        ...
        <p className={styles.toolDescription}>
          Sử dụng để phân tích và tổng hợp thông tin từ các tài liệu học thuật
          đã upload (Hiến pháp 2013, văn kiện Đại hội XIII, giáo trình MLN131)
          về các khái niệm: Nhà nước xã hội chủ nghĩa và Dân chủ xã hội chủ
          nghĩa.
        </p>
      ...
      <div className={styles.toolCategory}>
        ...
        <p className={styles.toolDescription}>
          Sử dụng để tra cứu và giải thích các khái niệm lý thuyết về Nhà nước
          pháp quyền xã hội chủ nghĩa và mối quan hệ giữa các khái niệm chính
          trị.
        </p>
      ...
      references: [
        {
          ...
          description:
            "Văn kiện Đại hội XIII của Đảng, các nghị quyết và tài liệu về xây dựng nhà nước pháp quyền XHCN",
        },
        {
          id: "admin-2",
          ...
          description:
            "Thông tin chính thức về tổ chức và hoạt động của Chính phủ, các chính sách và pháp luật hiện hành",
        },
        {
          id: "admin-3",
          ...
          description:
            "Tài liệu về quy trình xây dựng chính sách, pháp luật và cơ chế tham gia của nhân dân",
        },
      ],
    },
    {
      ...
      references: [
        {
          ...
          description:
            "Giáo trình K-2021 - Các khái niệm cơ bản về Nhà nước xã hội chủ nghĩa, Dân chủ xã hội chủ nghĩa và Nhà nước pháp quyền xã hội chủ nghĩa",
        },
        {
          id: "other-2",
          ...
          description:
            "Hiến pháp 2013 (sửa đổi bổ sung 2024), các văn bản pháp luật và tài liệu tham khảo về hệ thống pháp luật Việt Nam",
        },
      ],
    },
  ];
  ...
  return (
    <section className={styles.section}>
      ...
      <p className={styles.sectionDescription}>
        Tất cả thông tin trong game đều được kiểm chứng và tham khảo từ các
      ...
      {referenceGroups.map((group, groupIdx) => (
        <div key={groupIdx} className={styles.referenceGroup}>
          ...
          <p className={styles.referenceGroupDescription}>
...
const Section3_CreativeApplication: React.FC = () => {
  ...
  return (
    <section className={styles.section}>
      ...
      <p className={styles.sectionDescription}>
        Dự án này thể hiện sự kết hợp hài hòa giữa AI và con người trong quá
        trình phát triển:
      </p>
      ...
      <div className={styles.synthesisBox}>
        <h4>Tổng hợp</h4>
        <p>
          AI đóng vai trò là công cụ hỗ trợ mạnh mẽ, tăng tốc độ phát triển và
          đề xuất giải pháp. Con người là người quyết định cuối cùng, đảm bảo
          chất lượng, tính chính xác, và phù hợp văn hóa. Sự kết hợp này tạo ra
          sản phẩm vừa hiệu quả vừa có giá trị giáo dục cao.
        </p>
      </div>
    </section>
  );
    ...
    <section className={styles.section}>
      ...
      <div className={styles.commitmentList}>
        {commitments.map((commitment, idx) => (
          <div key={idx} className={styles.commitmentCategory}>
            <h3 className={styles.commitmentCategoryTitle}>
              {commitment.category}
            </h3>
      ...
      <div className={styles.signatureBox}>
        <p className={styles.signatureText}>
          Chúng tôi, các thành viên của nhóm, cam kết tuân thủ đầy đủ các nguyên
          tắc liêm chính học thuật nêu trên và chịu trách nhiệm hoàn toàn về
          tính trung thực của dự án này.
        </p>
      ...
      <header className={styles.pageHeader}>
        ...
        <div className={styles.headerContent}>
          ...
          <p className={styles.courseName}>
            Môn học: MLN131 - Chủ nghĩa Xã hội khoa học
          </p>
        </div>
      </header>
